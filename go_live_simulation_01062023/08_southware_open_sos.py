...
def read_transform(date_time, relations_dict, onedrive_path):
    df = pd.read_excel(
        onedrive_path + "Complete Master/SLSORDRH01-SAC-01-06-2023.xlsx")
    print(df)
    print(df["Customer Number"])
    print(df["Bill To Customer"])
    ...
    df.rename(columns={"Date Created": "Order Date",
              "Date Created": "Document Date", "Comment": "Work Description", "Ship-to Address 1": "Ship-to Address"}, inplace=True)
    ...
    df_columns = ["Document Type", "No.", "Sell-to Customer No.", "Bill-to Customer No.", "Your Reference", "Ship-to Code", "Ship-to Address", "Ship-to Address 2", "Ship-to City", "Ship-to State", "Ship-to ZIP Code", "Ship-to Country/Region Code", "Order Date", "Posting Date", "Shipment Date", "Payment Terms Code", "Shipment Method Code", "Location Code",
                  "Shortcut Dimension 1 Code", "Customer Posting Group", "Salesperson Code", "Gen. Bus. Posting Group", "Document Date", "External Document No.", "Payment Method Code", "Tax Area Code", "Tax Liable", "Work Description", "EVI BU Code (Dimension)", "Opptype Code (Dimension)", "SLEPRS Code (Dimension)"]
    ...
    df.to_excel(onedrive_path + "Download Southware/open_sos_headers_output" +
                date_time.strftime("%m%d%y") + ".xlsx", index=False)
    df.to_csv(onedrive_path + "Download Southware/open_sos_headers_output" +
