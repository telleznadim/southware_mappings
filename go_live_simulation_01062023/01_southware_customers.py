...
def read_transform(onedrive_path):
    df = pd.read_excel(
        onedrive_path + "Complete Master/CUSTOMER01-SAC-01-06-2023.xlsx")
    print(df)
    ...
    # df.to_excel(onedrive_path + "Download Southware/customers_output" +
    #             date_time.strftime("%m%d%y") + ".xlsx", index=False)
    # df.to_csv(onedrive_path + "Download Southware/customers_output" +
...
def main():
    onedrive_path = "/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/07-SAC/Go Live Simluation 01.06.2023/Customer/"
    # read_transform(onedrive_path)
    set_addr1_from_addr2()
