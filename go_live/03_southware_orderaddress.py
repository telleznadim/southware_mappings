...
def read_transform(onedrive_path):
    df = pd.read_excel(
        onedrive_path + "Complete Master/Z050ORDADR01.xlsx")
    print(df)
    ...
    df.to_excel(onedrive_path + "Download Southware/order_address_output" +
                date_time.strftime("%m%d%y") + ".xlsx", index=False)
    df.to_csv(onedrive_path + "Download Southware/order_address_output" +
...
onedrive_path = "/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/07-SAC/Go Live Simluation 01.06.2023/Order Address/"
