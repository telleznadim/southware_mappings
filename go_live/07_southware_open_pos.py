...
def read_transform(date_time, relations_dict, onedrive_path):
    df = pd.read_excel(
        onedrive_path + "Complete Master/PRCHORDH01-SAC-01-27-2023-426PM.xlsx")
    print(df)
    df = df[((df["Date Ordered"] >= "01/01/2022") &
             (df["Date Ordered"] <= "01/27/2023")) & (df["Status"] == "O")]
    # df = df[(df["Status"] == "O")]
    print(f'POs after filter...\n', df)
    print(df["Buyer Name"])
...
def main():
    ...
    onedrive_path = "/Users/nadimtellezbarrera/Library/CloudStorage/OneDrive-EVI/07-SAC/Go Live/6. Purchase Order Header-Lines/"
