...
def read_get_lines_from_headers(onedrive_path, date_time):
    df = pd.read_excel(
        onedrive_path + "Complete Master/SLSORD-Headers-N-01-30-2023-253PM.xlsx", dtype={"Misc Charge Code 1": str, "Misc Charge Code 2": str, "Misc Charge Code 3": str, "Misc Charge Code 4": str})
    # print(df)
...
def read_transform_lines(date_time, relations_dict, onedrive_path):
    df = pd.read_excel(
        onedrive_path + "Complete Master/SLSORD-Lines-N-01-30-2023-253PM.xlsx", dtype={"Stock # Ordered": str})
    print(df)
    df2 = read_get_lines_from_headers(onedrive_path, date_time)
    ...
    get_mapping_columns = [
        ...
        {"bc_column": "Order type", "source_column": "Document No.",
         "mapping_table": relations_dict["open_sos_headers_output"], "default_value": "Order", "old_value_column_name": "No.", "new_value_column_name": "Order Type"},
    ...
    print(f'Deleting SO Lines with 0 or less Quantity and with no header..')
    # df = df[(df["Quantity"] > 0) & df["header_exists"] != False]
    # df = df[df["header_exists"] != False]
    ...
    df.to_excel(onedrive_path + "Download Southware/open_sos_lines_output_review_" +
...
def delete_headers_if_not_exist_lines(date_time, onedrive_path):
    ...
    print(df_so_headers)
    df_so_headers.to_excel(onedrive_path + "Download Southware/open_sos_headers_output_review_" +
                           date_time.strftime("%m%d%y") + ".xlsx", index=False)
