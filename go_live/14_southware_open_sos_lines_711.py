...
def read_get_lines_from_headers(onedrive_path, date_time):
    df = pd.read_excel(
        onedrive_path + "Complete Master/SLSORDRH01-SAC-01-30-2023-939am.xlsx", dtype={"Misc Charge Code 1": str, "Misc Charge Code 2": str, "Misc Charge Code 3": str, "Misc Charge Code 4": str})
    # print(df)
...
def read_transform_lines(date_time, relations_dict, onedrive_path):
    df = pd.read_excel(
        onedrive_path + "Complete Master/SLSORDLN-01-30-2023-944AM.xlsx", dtype={"Stock # Ordered": str})
    ...
    print(df)
    df.to_excel(onedrive_path + "Download Southware/open_sos_lines_output_quotes_" +
...
def delete_headers_if_not_exist_lines(date_time, onedrive_path):
    ...
    print(df_so_headers)
    df_so_headers.to_excel(onedrive_path + "Download Southware/open_sos_headers_output_quotes_" +
                           date_time.strftime("%m%d%y") + ".xlsx", index=False)
