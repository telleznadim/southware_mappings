...
def read_transform(date_time, relations_dict):
    df = pd.read_excel(
        "files/from_southware/OPENPOHDUMP01-Southware-12-01-2022.xlsx")
    print(df)
    ...
    df_columns = ["Document Type",	"No.",	"Buy-from Vendor No.",	"Your Reference",	"Ship-to Name",	"Ship-to Address",	"Ship-to Address 2",	"Ship-to City",	"Ship-to Contact",	"Order Date",	"Posting Date",	"Payment Terms Code",	"Due Date",	"Payment Discount %",	"Shipment Method Code",	"Location Code",
                  "Shortcut Dimension 1 Code",	"Shortcut Dimension 2 Code",	"Vendor Posting Group",	"Purchaser Code",	"Gen. Bus. Posting Group",	"Ship-to ZIP Code",	"Ship-to State",	"Document Date",	"Payment Method Code",	"Tax Area Code",	"Dimension Set ID",	"Assigned User ID",	"IRS 1099 Code",	"EVI BU"]
...
def main():
    ...
    relations_dict = {"vendor_ids": vendor_ids,
                      "payment_terms_code": payment_terms_code, "location_code": location_code, "purchaser_code": purchaser_code, "shipment_method_code": shipment_method_code}
    # relations_dict = {}
